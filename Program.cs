...
namespace Dastan
{
    ...
    class Dastan
    {
        ...
        {
            int StartSquareReference = 0, FinishSquareReference = 0;
            ...
            while (!SquareIsValid)
            {
                FinishSquareReference = RGen.Next(NoOfColumns * NoOfRows);
        ...
        public void PlayGame()
        {
            ...
            while (!GameOver)                                                                           // looping while gameOver is set to false, loop is exited once game ends
            {
                ...
                bool SquareIsValid = false, usedSpaceJump = false;                                                             // setting that the square is not valid
                ...
                {
                    string SpaceJump = CurrentPlayer.HasSpaceJump() == true ? ", 5 to use your spacejump " : "";
                    Console.Write($"Choose move option to use from queue (1 to 3){SpaceJump} or 9 to take the offer: ");    // displaying instructions to the user
                    ...
                    else if (Choice == 5 && CurrentPlayer.HasSpaceJump() == true)
                    {
                        CurrentPlayer.UsedSpaceJump();
                        DoSpaceJump();
                        usedSpaceJump = true;
                        break;
                    }
                ...
                if (!usedSpaceJump)
                {
                    ...
                    while (!MoveLegal)
                    {
                        ...
                        MoveLegal = CurrentPlayer.CheckPlayerMove(Choice, StartSquareReference, FinishSquareReference);    // move is legal if there is a valid move in the move option that takes the piece from the start positon to the finish position
                        if (!MoveLegal)
                        {
                            Console.WriteLine("That move cannot be made \nPlease input new square references");
                        }
                    }
